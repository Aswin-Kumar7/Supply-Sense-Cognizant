/**
 * ProvenanceTag
 * Small inline chip indicating the source of a data point or recommendation.
 *
 * Usage:
 *   <ProvenanceTag type="rule" />          → ⚡️ Rule-based
 *   <ProvenanceTag type="ai" />            → ✦ Claude
...
export function ProvenanceTag({ type, label, size = 'sm' }: ProvenanceTagProps) {
  ...
  const icon = isRule ? '⚡️' : '✦'
  const text = label ?? (isRule ? 'Rule-based' : 'Claude')
  ...
  return (
    <span
      title={isRule ? 'Computed by rule engine' : 'Generated by Claude AI'}
      style={{
        display: 'inline-flex',
        alignItems: 'center',
        gap: '3px',
